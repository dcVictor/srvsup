...
const handleFullRefresh = async () => {
  loading.value = true;
  try {
    const responseRefresh = await fetch('http://192.168.1.53:3000/api/printersrefresh');
...
const fetchHistory = async () => {
  ...
  try {
    const response = await fetch(`http://192.168.1.53:3000/api/historico?dataInicial=${startDate.value}&dataFinal=${endDate.value}`);
